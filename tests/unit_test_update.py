...
#         assert status == 200, f"Expected HTTP status code 200, got {status}"
    ...
    date0 = "20221"
    date1 = "2022-02-02T15:00:00Z" # 1 элем
    date2 = "2022-02-02T12:00:00Z" # 2 элем
    print("test if wrong date")
    status, _ = request(f"/updates?date={date0}", json_response=True)
    assert status == 400, f"Expected HTTP status code 400, got {status}"
    print("test passed: if wrong date")
    print("test if 2")
    status, response = request(f"/updates?date={date1}", json_response=True)
    assert status == 200, f"Expected HTTP status code 200, got {status}"
    assert len(response["items"]) == 2, f"Expected 2, got {len(response)}"
    print("test passed: if 2")
    print("test if 2")
    status, response = request(f"/updates?date={date2}", json_response=True)
    assert status == 200, f"Expected HTTP status code 200, got {status}"
    assert len(response["items"]) == 2, f"Expected 2, got {len(response)}"
    print("test passed: if 2")
