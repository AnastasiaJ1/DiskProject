...
IMPORT_BATCHES_1 = [
    ...
    {#- check if delete one item with child
        "items": [
            {
                "type": "FOLDER",
                "id": "a3",
...
def test_delete_full():
    print("test  импортируем необходимые элементы")
...
#
#         assert status == 200, f"Expected HTTP status code 200, got {status}"
    print("Test passed: импортируем необходимые элементы")

    params = urllib.parse.urlencode({
            "date": "2022-02-04T01:00:00Z"
    })
    print("test check if delete one item")
    status, _ = request(f"/delete/a2?{params}", method="DELETE")
    status, response1 = request(f"/nodes/a1", json_response=True)
    assert status == 200, f"Expected HTTP status code 200, got {status}"
    status, response = request(f"/nodes/a2", json_response=True)
    assert status == 404, f"Expected HTTP status code 404, got {status}"
    exp = "2022-02-04T01:00:00Z"
    g = response1["date"]
    assert response1["date"] == exp ,f"Expected {exp}, got {g}"
    print("Test passed: check if delete one item")

    status, _ = request(f"/delete/a1?{params}", method="DELETE")

    print("test check if delete one item with child")
    status, _ = request(f"/delete/a3?{params}", method="DELETE")
    status, response1 = request(f"/nodes/a3", json_response=True)
    assert status == 404, f"Expected HTTP status code 404, got {status}"
    status, response = request(f"/nodes/a4", json_response=True)
    assert status == 404, f"Expected HTTP status code 404, got {status}"
    status, response = request(f"/delete/nodesAllh", json_response=True)
    assert response==None, f"Expected None, got {response}"
    print("Test passed: check if delete one item with child")
